...
app.get('/api/todos/:id', (req, res) => {
  ...
  const todoList = users.getUserData(username);
  console.log("from server app.get('/api/todos/:id': ",todoList);
  const { id } = req.params;
...
app.put('/api/todos/:id', (req, res) => {
  ...
  const todoList = users.getUserData(username);
  console.log("from server: app.put('/api/todos/:id' ",todoList);
  const { id } = req.params;
...
app.patch('/api/todos/:id', (req, res) => {
  ...
  const todoList = users.getUserData(username);
  //console.log("from server: app.patch('/api/todos/:id' ",todoList);
  console.log("from server: app.put('/api/todos/:id' ",todoList.getTodos());
  if(!todoList.contains(id)) {
    res.status(404).json({ error: `noSuchId`, message: `No todo with id ${id}` });
    return;
  }
